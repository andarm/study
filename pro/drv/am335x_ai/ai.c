...
static int ad_setup_reg(void)
{
    ...
    if (!ts_dev->tsc_base) {
        pr_debug("failed to map registers.\n");
        return -1;
    }
#if  1
    ...
    if (IS_ERR(clk)) {
        pr_debug("failed to get TSC fck\n");
    }

    clock_rate = clk_get_rate(clk);
    clk_value = clock_rate / ADC_CLK;
    if (clk_value < 7) {
       pr_debug( "clock input less than min clock requirement\n");
    }
 ...
 //   if(!ev_adc)
    {
        ...
        else
        {
            ...
            ch1 = (u16)read_ch_val(1);
            ch2 = (u16)read_ch_val(2);
            ch3 = (u16)read_ch_val(3);
            calc_result(&ch1,1);
            calc_result(&ch2,2);
 	    calc_result(&ch3,3);
...
#define lin_test 1 
    ...
    // 为该类 创建一个设备
    pst_ad_drv->dev = device_create(pst_ad_drv->cls, NULL, MKDEV(ad_major, 0), NULL, "%s", ad_dev_name);
