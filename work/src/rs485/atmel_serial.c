...
/* Added by MYIR */
static volatile long open_flags[BITS_TO_LONGS(ATMEL_MAX_UART)];
...
/**********************************SAFTOP******************************/
	...
	if(port->line == 6)
	{
		at91_set_gpio_value(AT91_PIN_PC13, 0);
	}
	// USART-RS485-2
	else if(port->line == 3)
	{
...
/*******************************SAFTOP*****************************/
	...
	if(port->line == 6)
	{
		at91_set_gpio_value(AT91_PIN_PC13, 1);
	}
	// USART2-RS485-2
	else if(port->line == 3)
	{
...
static void atmel_rx_request_dma(struct atmel_uart_port *atmel_port)
{
	...
	if (atmel_use_dma_rx(port) && pdata->dma_rx_slave) {
		...
		dev_dbg(port->dev, "%s: RX: got channel %d\n",
			__func__,
			chan->chan_id);
		/* Added by MYIR, debug only
		printk(KERN_ERR"\r\n%s %s: RX: got channel %d\n",
			dev_name(port->dev),
			__func__,
			chan->chan_id); */
	}
	...
	if (chan) {
		...
		if (!nent) {
			dev_dbg(port->dev, "need to release resource of dma\n");
			/* Added by MYIR, debug only
			printk(KERN_ERR"need to release resource of dma\n"); */
		} else {
			dev_dbg(port->dev, "%s: mapped %d@%p to %x\n",
				__func__,
			...
			/* Added by MYIR, debug only
			printk(KERN_ERR"%s %s: mapped %d@%p to %x\n",
				dev_name(port->dev),
				__func__,
				sg_dma_len(&atmel_port->sg_rx),
				ring->buf,
...
static void atmel_tasklet_func(unsigned long data)
{
	...
	/* Added by MYIR. */
	if(!test_bit(port->line, open_flags)) {
		printk(KERN_ALERT "[MYIR_DBG] port %d aready closed.\n", port->line);
		return;
	}
...
static void atmel_shutdown(struct uart_port *port)
{
	...
	/* Added by MYIR. */
	clear_bit(port->line, open_flags);
